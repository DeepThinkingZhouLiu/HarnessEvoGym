...
async function readTestManifest(path, expectedSha256) {
  let contents
  try {
  ...
  if (ids.length !== TEST_TASK_COUNT || new Set(ids).size !== ids.length
      || ids.some((id) => !INSTANCE_PATTERN.test(id))
...
function validatePartitionResult(result, candidateId, instanceIds) {
  if (!isPlainObject(result) || !isPlainObject(result.summary) || !Array.isArray(result.records)) {
    throw brokerError('SEALED_OUTPUT_INVALID')
  }
  const { summary, records } = result
  if (summary.candidateId !== candidateId || summary.total !== TEST_TASK_COUNT
      || !Number.isInteger(summary.verified) || summary.verified < 0
      || summary.verified > TEST_TASK_COUNT || records.length !== TEST_TASK_COUNT) {
    throw brokerError('SEALED_OUTPUT_INVALID')
  }
  let resolved = 0
...
function serializeResult(result, receipt, secretValues = []) {
  let summaryText
  let recordsText
  try {
...
export async function runSealedTest({
  ...
  try {
    ...
    const instanceIds = await readTestManifest(testManifestPath, testManifestSha256)
    ...
    const result = validatePartitionResult(rawResult, candidateId, instanceIds)
    const receipt = makeReceipt(candidateId, now, makeReceiptId)
    const serialized = serializeResult(result, receipt, secretValues)

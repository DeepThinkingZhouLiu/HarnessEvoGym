...
const LEVELS = new Set(['baseline', 'l1', 'l2', 'l3', 'unselected'])
...
// being reinterpreted as Markdown delimiters. Keep model-authored mutation text
...
function csvCell(value) {
  ...
  // curve.json remains the lossless source for the original mutation text.
...
function indexTestAggregates(testAggregates, expectedTotal, errors) {
  return indexAggregates(testAggregates, expectedTotal, 'testAggregates', errors).byCandidate
}

function indexValidationAggregates(validationAggregates, expectedTotal, errors) {
...
function indexMutations(mutations, errors) {
  const byCandidate = new Map()
  ...
  for (const [index, mutation] of mutations.entries()) {
    ...
    if (!hasText(mutation.candidateId)) {
      errors.push(`${path}.candidateId must be a non-empty string`)
      continue
    }
    if (byCandidate.has(mutation.candidateId)) {
      errors.push(`duplicate mutation ${mutation.candidateId}`)
      continue
    }
    if (!hasText(mutation.direction)) errors.push(`${path}.direction must be a non-empty string`)
    const decidedMilliseconds = parseTimestamp(mutation.decidedAt, `${path}.decidedAt`, errors)
    byCandidate.set(mutation.candidateId, { ...mutation, decidedMilliseconds })
  }
  return byCandidate
}
...
function levelAttemptSummaries(points) {
  ...
  if (points.some((point) => point.level === 'unselected')) levels.push('unselected')
  const summaries = new Map(levels.map((level) => [level, {
    level,
    attempts: 0,
    completed: 0,
    candidateFailures: 0,
    promoted: 0,
    rejected: 0,
    startingIncumbentVerified: null,
    endingIncumbentVerified: null,
    validationGain: 0,
    bestAttemptVerified: null,
  }]))
  let incumbentVerified = points[0].validation.verified
  for (const point of points.slice(1)) {
    const summary = summaries.get(point.level)
    if (!summary) continue
    if (summary.startingIncumbentVerified === null) {
      summary.startingIncumbentVerified = incumbentVerified
    }
    summary.attempts += 1
    if (point.outcome === 'completed') summary.completed += 1
    if (point.outcome === 'candidate_failure') summary.candidateFailures += 1
    if (point.decision === 'promoted') {
      summary.promoted += 1
      summary.validationGain += point.validation.verified - incumbentVerified
      incumbentVerified = point.validation.verified
    } else if (point.decision === 'rejected') {
      summary.rejected += 1
    }
    summary.endingIncumbentVerified = summary.startingIncumbentVerified + summary.validationGain
    summary.bestAttemptVerified = summary.bestAttemptVerified === null
      ? point.validation.verified
      : Math.max(summary.bestAttemptVerified, point.validation.verified)
  }
  for (const summary of summaries.values()) {
    if (summary.startingIncumbentVerified === null) {
      summary.startingIncumbentVerified = incumbentVerified
      summary.endingIncumbentVerified = incumbentVerified
    }
  }
 ...
 * the Controller's append-only mutation log.
 * Candidate ordering always comes from CampaignState.candidates; test scores are
 * never consulted for ordering or selection.
 */
export function buildCampaignReport({ campaignState, validationAggregates, testAggregates, mutations }) {
  const errors = []
  ...
  const validationTotal = campaignState.partitionTotals?.validation ?? VALIDATION_TOTAL
  const testTotal = campaignState.partitionTotals?.test ?? TEST_TOTAL
  if (!Number.isSafeInteger(validationTotal) || validationTotal < 1
      || !Number.isSafeInteger(testTotal) || testTotal < 1) {
    errors.push('campaignState.partitionTotals must contain positive integers when present')
  }
  ...
  const testByCandidate = indexTestAggregates(testAggregates, testTotal, errors)
  const validationIndex = indexValidationAggregates(validationAggregates, validationTotal, errors)
  const validationByCandidate = validationIndex.byCandidate
  const mutationByCandidate = indexMutations(mutations, errors)
  const layerSelection = campaignState.layerSelection ?? 'controller-sequential'
  if (!['controller-sequential', 'updater-soft'].includes(layerSelection)) {
    errors.push('campaignState.layerSelection is invalid')
  }
  ...
  const usedMutationCandidates = new Set()
  ...
  for (const [index, candidate] of (Array.isArray(candidates) ? candidates : []).entries()) {
    ...
    if (!LEVELS.has(candidate.level)
        || (candidate.level === 'unselected' && layerSelection !== 'updater-soft')) {
    ...
    const testEvaluated = candidate.testEvaluated ?? isObject(receipt)
    if (testEvaluated) {
      if (!isObject(receipt) || !hasText(receipt.receiptId)) {
        errors.push(`${path}.testReceipt.receiptId must be non-empty when testEvaluated is true`)
      } else if (receiptIds.has(receipt.receiptId)) {
        errors.push(`duplicate test receipt ${receipt.receiptId}`)
      } else {
        receiptIds.add(receipt.receiptId)
      }
    } else if (receipt !== undefined && receipt !== null) {
      errors.push(`${path}.testReceipt must be absent when testEvaluated is false`)
    }

    const test = testByCandidate.get(candidate.candidateId)
    if (testEvaluated && !test) {
      errors.push(`missing test aggregate for candidate ${candidate.candidateId}`)
    } else if (!testEvaluated && test) {
      errors.push(`unexpected test aggregate for unscheduled candidate ${candidate.candidateId}`)
    ...
    let mutation = null
    if (index > 0) {
      mutation = mutationByCandidate.get(candidate.candidateId) ?? null
      if (!mutation) {
        errors.push(`missing mutation for candidate ${candidate.candidateId}`)
      } else {
        usedMutationCandidates.add(candidate.candidateId)
        if (mutation.level !== candidate.level) {
          errors.push(`mutation ${candidate.candidateId} level does not match`)
        }
        if (mutation.decision !== candidate.decision) {
          errors.push(`mutation ${candidate.candidateId} decision does not match`)
        }
        if (mutation.validationScore !== candidate.validationVerified) {
          errors.push(`mutation ${candidate.candidateId} validation score does not match`)
        }
        if (mutation.decidedMilliseconds !== null && evaluatedMilliseconds !== null
            && mutation.decidedMilliseconds < evaluatedMilliseconds) {
          errors.push(`mutation ${candidate.candidateId} was logged before evaluation`)
        }
      }
    }
    ...
    points.push({
      ...
      mutationCommit: mutation?.commit ?? null,
      mutationDirection: mutation?.direction ?? null,
      mutationDecidedAt: mutation?.decidedAt ?? null,
  ...
  for (const candidateId of mutationByCandidate.keys()) {
    if (!usedMutationCandidates.has(candidateId)) {
      errors.push(`mutation ${candidateId} is not used by a candidate`)
    }
  }
  ...
  return {
    ...
    smoothed: false,
    benchmark: campaignState.benchmark ?? 'putnambench-lean',
    validationTotal,
    testTotal,
    textExportPolicy: {
      jsonMutationText: 'raw',
...
export function renderCurveCsv(report) {
  ...
  const rows = report.points.map((point) => [
    ...
    point.parentId,
    point.level,
    point.mutationCommit,
    point.mutationDirection,
    point.mutationDecidedAt,
    point.evaluatedAt,
    point.elapsedHours,
    point.outcome,
    point.decision,
    ...
    point.test?.verified,
    point.test?.total,
    point.test ? round(point.test.rate) : null,
...
export function renderCurveSvg(report) {
  ...
  const bandColors = {
    l1: '#eff8ff', l2: '#f4f3ff', l3: '#fff4ed', unselected: '#f2f4f7',
  }
  const lines = []
  ...
  lines.push(`<text x="${left}" y="38" font-family="sans-serif" font-size="24" font-weight="700" fill="#101828">${escapeXml(report.benchmark)} Harness RSI</text>`)
  ...
  const validationPoints = report.points
    ...
    .join(' ')
  const testSeries = report.points.filter((point) => point.test !== null)
  const testPoints = testSeries
  ...
  for (const point of testSeries) {
    lines.push(svgMarker(point, x(point.elapsedHours), y(point.test.rate), testColor))
  }

  const legendY = 678
  lines.push(`<line x1="${left}" y1="${legendY}" x2="${left + 28}" y2="${legendY}" stroke="${validationColor}" stroke-width="3"/><text x="${left + 36}" y="${legendY + 5}" font-family="sans-serif" font-size="13" fill="#344054">Validation (${report.validationTotal})</text>`)
  lines.push(`<line x1="${left + 170}" y1="${legendY}" x2="${left + 198}" y2="${legendY}" stroke="${testColor}" stroke-width="3"/><text x="${left + 206}" y="${legendY + 5}" font-family="sans-serif" font-size="13" fill="#344054">Hidden test (${report.testTotal})</text>`)
...
export function renderImprovementsMarkdown(report) {
  const lines = [
    ...
    '',
    'Rows are in campaign-ledger order. Each direction comes from the Updater commit; hidden-test scores were unsealed only after campaign closure and were never used for acceptance, ordering, or stopping.',
    'Dynamic Markdown text is rendered literally. Spreadsheet-formula-like CSV text is prefixed with an apostrophe; `curve.json` retains the original mutation text.',
    '',
    '| # | elapsed h | level | candidate | result | mutation direction | validation | hidden test |',
    '|---:|---:|:---:|---|:---:|---|---:|---:|',
  ]
  for (const point of report.points) {
    const direction = point.mutationDirection ?? 'Frozen baseline'
    const testScore = point.test === null ? 'n/a' : `${point.test.verified}/${point.test.total}`
    lines.push(
      `| ${point.sequence} | ${point.elapsedHours} | ${point.level.toUpperCase()} | \`${escapeMarkdownCell(point.candidateId)}\` | ${point.marker} | ${escapeMarkdownCell(direction)} | ${point.validation.verified}/${point.validation.total} | ${testScore} |`,
    )
  }

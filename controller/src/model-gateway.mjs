...
function forceTrustedRequestFields(body, maxOutputTokens) {
...
function normalizeOptions(options) {
  ...
  const maxRequests = requirePositiveInteger(options.maxRequests ?? DEFAULT_MAX_REQUESTS, 'maxRequests')
  const maxOutputTokens = requirePositiveInteger(
    options.maxOutputTokens ?? DEFAULT_MAX_OUTPUT_TOKENS,
    'maxOutputTokens',
  )
  if (maxOutputTokens > 65_536) throw new TypeError('maxOutputTokens must be at most 65536')
...
export async function startModelGateway(options) {
  const { host = '127.0.0.1', port = 0, ...gatewayOptions } = options ?? {}
  const gateway = createModelGateway(gatewayOptions)
  return gateway.start({ host, port })
}

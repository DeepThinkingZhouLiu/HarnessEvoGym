...
async function fixture({
  ...
  const root = rootOverride ?? await mkdtemp(join(tmpdir(), 'production-runtime-'))
  const paths = {
    root,
    runtimes: join(root, 'runtimes'),
    runtimeCache: join(root, 'runtime-cache'),
    ...
    mutationTemplate: join(root, 'control', 'mutation.md'),
    feedback: join(root, 'campaign', 'private', 'validation'),
    evolutionLog: join(root, 'campaign', 'public', 'evolution-log.jsonl'),
    git: join(root, 'campaign', 'private', 'linear-git.git'),
  ...
  await Promise.all([
    ...
    mkdir(paths.feedback, { recursive: true }),
    mkdir(paths.git, { recursive: true }),
    mkdir(join(root, 'campaign', 'public'), { recursive: true }),
  ])
  await Promise.all([
    ...
    writeFile(paths.mutationTemplate, 'candidate={{ candidate.id }} level={{ mutation.level }} log={{ feedback.log }}\n'),
    writeFile(paths.evolutionLog, ''),
  ...
  const dependencies = {
    ...
    async mutationRunner(options) {
      const template = await readFile(options.templatePath, 'utf8')
      calls.mutations.push({ ...options, rendered: renderPrompt(template, options.templateValues) })
      calls.gatewayLifecycle.push(`agent:mutation:${new URL(options.invocationOptions.gatewayUrl).port}`)
      return {
        result: {
          ok: true,
          exitCode: 0,
          stdout: 'mutation committed',
          stderr: '',
          durationMs: 12,
  ...
  const runtime = new PutnamEvolutionRuntime({
    ...
    ...(runtimeCache ? { runtimeCacheRoot: paths.runtimeCache } : {}),
    ...
    mutationTemplatePath: paths.mutationTemplate,
...
test('content-addressed runtime cache reuses a frozen build across Controller processes', async () => {
  const digest = 'a'.repeat(64)
  const first = await fixture({ runtimeCache: true })
  const cold = await first.runtime.buildCandidate({
    candidateId: 'baseline',
    candidateRoot: first.paths.baselineSource,
    level: 'baseline',
    candidateDigest: digest,
  })
  assert.equal(cold.cacheHit, false)
  assert.equal(first.calls.processes.some((entry) => entry.args.includes('install')), true)

  const cacheEntries = await readdir(first.paths.runtimeCache)
  assert.deepEqual(cacheEntries, [cold.cacheKey])
  const attestation = JSON.parse(await readFile(join(
    first.paths.runtimeCache,
    cold.cacheKey,
    '.harness-rsi-runtime-cache-v1.json',
  ), 'utf8'))
  assert.equal(attestation.candidateDigest, digest)
  assert.equal(attestation.cacheKey, cold.cacheKey)
  assert.equal(await realpath(cold.runtimeRoot), join(first.paths.runtimeCache, cold.cacheKey))

  const second = await fixture({ rootOverride: first.root, runtimeCache: true })
  const warm = await second.runtime.buildCandidate({
    candidateId: 'baseline',
    candidateRoot: second.paths.baselineSource,
    level: 'baseline',
    candidateDigest: digest,
  })
  assert.equal(warm.cacheHit, true)
  assert.equal(warm.cacheKey, cold.cacheKey)
  assert.equal(second.calls.processes.length, 0)
  assert.equal(second.calls.copies.length, 0)
  assert.equal(second.calls.storeValidations.length, 0)
  assert.equal(await realpath(warm.runtimeRoot), join(second.paths.runtimeCache, warm.cacheKey))
})

test('runtime cache invalidates on source digest changes and fails closed on attestation tampering', async () => {
  const context = await fixture({ runtimeCache: true })
  const first = await context.runtime.buildCandidate({
    candidateId: 'baseline',
    candidateRoot: context.paths.baselineSource,
    level: 'baseline',
    candidateDigest: 'a'.repeat(64),
  })
  const changed = await context.runtime.buildCandidate({
  ...
  assert.equal(context.calls.processes.filter((entry) => entry.args.includes('install')).length, 2)

  const metadataPath = join(
    context.paths.runtimeCache,
    first.cacheKey,
    '.harness-rsi-runtime-cache-v1.json',
  )
  ...
  await assert.rejects(
    ...
    (error) => error instanceof ProductionRuntimeError
      && /runtime build infrastructure failure/iu.test(error.message),
  )
})
...
test('Updater mutates once using the frozen baseline runtime and one fresh gateway', async () => {
  const context = await fixture()
  const baseline = await context.runtime.buildCandidate({
    candidateId: 'baseline', candidateRoot: context.paths.baselineSource, level: 'baseline',
  })
  const mutation = await context.runtime.mutate({
    campaignId: 'campaign',
    candidateId: 'c0001-l1',
    parentId: 'baseline',
    level: 'l1',
    candidateRoot: context.paths.candidateSource,
    gitRoot: context.paths.git,
    feedbackRoot: context.paths.feedback,
    evolutionLogPath: context.paths.evolutionLog,
  })

  assert.equal(mutation.durationMs, 12)
  assert.equal(context.calls.mutations.length, 1)
  const phase = context.calls.mutations[0]
  assert.match(phase.rendered, /candidate=c0001-l1 level=l1/u)
  assert.match(phase.rendered, /\/opt\/harness-rsi\/evolution-log\.jsonl/u)
  ...
  assert.equal(phase.invocationOptions.gitRoot, context.paths.git)
...
test('Updater callback failures after the latest provider/credential audit pause as infrastructure', async (context) => {
  ...
  const mutation = (paths) => ({
    ...
    evolutionLogPath: paths.evolutionLog,
  })

  await context.test('credential failure during mutation', async () => {
    const callbackError = new Error('Updater did not commit a mutation')
    const fixtureContext = await fixture({
      dependencyOverrides: {
        startGateway: auditedGateway([{ status: 502, origin: 'credential' }]),
        async mutationRunner() { throw callbackError },
      },
    })
    ...
    await assert.rejects(
      () => fixtureContext.runtime.mutate(mutation(fixtureContext.paths)),
      (error) => error instanceof ProductionRuntimeError
        && error.kind === 'infrastructure'
        && error.operation === 'updater-mutation'
        && error.cause === callbackError,
    )
  })

  await context.test('upstream 503 during mutation', async () => {
    const callbackError = new Error('Updater mutation session exited nonzero')
    const fixtureContext = await fixture({
      dependencyOverrides: {
  ...
  await context.test('only the final audit controls classification', async () => {
    const callbackError = new Error('candidate-side mutation failure')
    const fixtureContext = await fixture({
      dependencyOverrides: {
...
test('production updater cannot bypass bwrap and uses the firewall-restricted updater identity', async () => {
  ...
  await context.runtime.mutate({
    ...
    gitRoot: context.paths.git, evolutionLogPath: context.paths.evolutionLog,
  })
  ...
  for (const invocation of executions) {
    ...
    assert.equal(explicitMounts(invocation.args).length, 8)
    assert.equal(mountMode(
      invocation.args,
    ...
    assert.equal(mountMode(
      invocation.args,
      context.paths.evolutionLog,
      '/opt/harness-rsi/evolution-log.jsonl',
    ), '--ro-bind')
    assert.equal(mountMode(
      invocation.args,
      context.paths.patch,
  ...
  assert.equal(updaterPrivateDirectories.length >= 3, true)
...
test('duplicate updater dummy credentials fail before starting another gateway', async () => {
  ...
  await context.runtime.mutate(mutation)
  await assert.rejects(
    () => context.runtime.mutate({ ...mutation, candidateId: 'c0002-l1' }),
    ProductionRuntimeError,
  )
  assert.equal(context.calls.gateways.length, 1)
})
...
test('unconfirmed Updater lease cleanup sticky-poisons the UID against port reuse', async () => {
  ...
  await assert.rejects(
    () => context.runtime.mutate(mutation),
    (error) => error instanceof ProductionRuntimeError && error.kind === 'infrastructure',
  )
  await assert.rejects(
    () => context.runtime.mutate({ ...mutation, candidateId: 'c0002-l1' }),
...
test('production runtime requires bwrap/verifier identity and gates legacy runner injection', async () => {
  ...
  await assert.rejects(
    ...
    /custom mutationRunner requires legacyUnsafeExecution/u,
  )
  await assert.rejects(

...
The run command uses only feedback and selection. The Controller trust-root paths must be committed before preflight/evolution; each run freezes the superproject SHA. Finalization locks the champion, replays feedback for a comparable training gain, and evaluates sealed final in one consumed attempt. Integrity and revision checks happen before unlock; an atomic create-if-absent `final-attempt.json` claim prevents concurrent finalizers from both entering replay. A failed attempt may be recovered exactly once with `--recover-infrastructure` only when the Controller proves that no sealed-final path or result was ever created. The original claim and failure remain auditable, incomplete public-feedback artifacts are archived, and an atomic recovery claim prevents a third attempt. A recovery finalizer may use a clean descendant Controller revision while recording both revisions; any configuration, source, or Candidate drift still fails closed.

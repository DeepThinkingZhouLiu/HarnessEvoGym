...
async function runOneTask(options, problemId, opaqueRunRoot) {
  ...
  try {
      ...
      try {
        ...
        if (lastSolver.status !== 'completed') {
          const retryable = lastSolver.failureKind === 'infrastructure'
            && retryCount < options.infrastructureRetries
        ...
        while (true) {
          ...
          if (lastVerifier.failureKind !== 'infrastructure'
              || retryCount >= options.infrastructureRetries) break
          retryCount += 1
        }
        break
      } finally {
    ...
    return normalizeRecord({
      ...
      verifierAttempts,
      usage: usageFromAudits(audits, gateway),
      latencyMs,
      traceRef,
    })
  } finally {
...
export async function runPartition({
  ...
  for (const instanceId of instanceIds) {
    if (!PROBLEM_ID.test(instanceId)) throw new ProtocolError(`非法 PutnamBench instanceId：${String(instanceId)}`)
  }

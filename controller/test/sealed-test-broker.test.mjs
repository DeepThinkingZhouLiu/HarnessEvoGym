...
test('HLE broker seals an opaque 50-task partition without returning IDs or scores', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sealed-hle-broker-'))
  const ids = Array.from({ length: 50 }, (_, index) => (
    `hle_${index.toString(16).padStart(24, '0')}`
  ))
  const manifestPath = join(root, 'test.ids')
  const sealedOutputPath = join(root, 'sealed', CANDIDATE_ID)
  const manifest = `${ids.join('\n')}\n`
  const manifestSha256 = createHash('sha256').update(manifest).digest('hex')
  await writeFile(manifestPath, manifest, { mode: 0o400 })
  const receipt = await runSealedTest({
    candidateId: CANDIDATE_ID,
    testManifestPath: manifestPath,
    testManifestSha256: manifestSha256,
    sealedOutputPath,
    expectedCount: 50,
    now: () => COMPLETED_AT,
    makeReceiptId: () => RECEIPT_ID,
    runPartition: async ({ instanceIds }) => ({
      summary: { candidateId: CANDIDATE_ID, verified: 31, total: 50 },
      records: recordsFor(instanceIds, 31),
    }),
  })
  assert.equal(JSON.stringify(receipt).includes('hle_'), false)
  assert.equal(Object.hasOwn(receipt, 'verified'), false)
  const internal = assertInternalSealedReceipt(JSON.parse(await readFile(
    join(sealedOutputPath, 'receipt.internal.json'),
    'utf8',
  )), CANDIDATE_ID, 50)
  assert.equal(internal.recordCount, 50)
  assert.equal(JSON.parse(await readFile(join(sealedOutputPath, 'summary.json'), 'utf8')).total, 50)
})

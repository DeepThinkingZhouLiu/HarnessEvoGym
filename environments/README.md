...
为每条 Trial 创建独占 AgentBay Session，运行时读取 AgentBay Key/Image/Policy，且不会把这些值暴露给
Candidate。Session 创建串行执行；已创建 Session 内的 Docker 准备、Solver 与 Verifier
可以并行运行。它也是无本地 Docker 场景的 Smoke 后端。

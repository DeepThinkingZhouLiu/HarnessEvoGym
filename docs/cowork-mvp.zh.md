...
| 模块                                      | 负责什么                                             | 不负责什么                         |
|-------------------------------------------|------------------------------------------------------|------------------------------------|
| `controller/src/orchestrator.mjs`         | 一轮/多轮编排、状态、谱系、晋升、一次性 Final        | 开放式失败归因                     |
...
| `controller/src/model-gateway.mjs`        | Run 级内部网络、一次性令牌与网关生命周期             | 决定 Agent 如何解题                 |
...
npm run rsi -- evolve finalize --run .rsi/runs/cowork-l1-smoke-001
```

...
function closedFixture(root, status = 'CLOSED') {
  ...
  const store = {
    ...
    async readEvolutionLog() {
      calls.mutations += 1
      return [{
        candidateId: 'candidate-l1', level: 'l1', decision: 'promoted',
        validationScore: 110, commit: 'c'.repeat(40),
        direction: 'strengthen proof search', decidedAt: '2026-01-01T02:00:00Z',
      }]
    },
  }
...
test('HLE status preserves public 50-task validation progress across re-projection', () => {
  const state = openState()
  state.benchmark = 'hle-text-math'
  state.partitionTotals = { validation: 50, test: 50 }
  state.incumbent.validationVerified = 31
  state.incumbent.validationTotal = 50
  state.candidates[0].validationVerified = 31
  state.candidates[0].validationTotal = 50
  const first = formatCampaignStatus(state)
  const second = JSON.parse(renderCampaignStatus(first))
  assert.deepEqual(second.partitionTotals, { validation: 50, test: 50 })
  assert.equal(second.benchmark, 'hle-text-math')
  assert.equal(second.incumbent.validationVerified, 31)
  assert.equal(second.candidates[0].validationTotal, 50)
})
...
test('known string fields are fail-safe redacted if state corruption injects hidden material', () => {
  ...
  state.candidates[0].candidateId = 'hle_0123456789abcdef01234567'
  assert.equal(renderCampaignStatus(formatCampaignStatus(state)).includes('hle_'), false)
})
...
test('non-terminal report request fails before vault or mutation-log access', async (context) => {
  for (const status of ['EVOLVING_L2', 'PAUSED_INFRASTRUCTURE', 'ABORTED_SECURITY']) {
    await context.test(status, async () => {
      ...
      const store = {
        ...
        async readEvolutionLog() { calls.push('mutations'); return [] },

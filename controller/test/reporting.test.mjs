...
function fixture() {
  const candidates = [
    ...
    },
    {
      ...
      outcome: 'completed', validationVerified: 110,
    ...
    },
    {
      ...
      outcome: 'completed', validationVerified: 108,
    ...
    },
    {
      ...
      outcome: 'candidate_failure', validationVerified: 105,
  ...
  return {
    ...
    mutations: [
      {
        candidateId: 'candidate-l1', level: 'l1', decision: 'promoted', validationScore: 110,
        commit: 'a'.repeat(40), direction: 'Tighten search, then verify',
        decidedAt: '2026-01-01T02:00:00Z',
      },
      {
        candidateId: 'candidate-l2', level: 'l2', decision: 'rejected', validationScore: 108,
        commit: 'b'.repeat(40), direction: 'Preserve context, retry tool errors',
        decidedAt: '2026-01-01T04:00:00Z',
      },
      {
        candidateId: 'candidate-l3', level: 'l3', decision: 'rejected', validationScore: 105,
        commit: null, direction: 'updater failure',
...
test('soft-layer report accepts non-monotonic updater choices and attributes gains chronologically', () => {
  const input = fixture()
  input.campaignState.layerSelection = 'updater-soft'
  const chosen = ['l2', 'l1', 'l3']
  input.campaignState.candidates.slice(1).forEach((candidate, index) => {
    candidate.level = chosen[index]
    input.mutations[index].level = chosen[index]
  })
  const report = buildCampaignReport(input)
  assert.equal(report.layerSelection, 'updater-soft')
  assert.deepEqual(report.points.slice(1).map((point) => point.level), chosen)
  const l2 = report.levelSummaries.find((summary) => summary.level === 'l2')
  const l1 = report.levelSummaries.find((summary) => summary.level === 'l1')
  assert.equal(l2.startingIncumbentVerified, 100)
  assert.equal(l2.validationGain, 10)
  assert.equal(l1.startingIncumbentVerified, 110)
  assert.equal(l1.validationGain, 0)
})
...
test('HLE report uses frozen 50/50 totals in validation and the curve legend', () => {
  const input = fixture()
  input.campaignState.benchmark = 'hle-text-math'
  input.campaignState.partitionTotals = { validation: 50, test: 50 }
  const validationScores = [30, 33, 32, 31]
  ...
  input.campaignState.candidates.forEach((candidate, index) => {
    candidate.validationVerified = validationScores[index]
    candidate.validationTotal = 50
    if (index > 0) input.mutations[index - 1].validationScore = validationScores[index]
  })
  input.validationAggregates.forEach((aggregate, index) => {
    aggregate.verified = validationScores[index]
    aggregate.total = 50
  })
  input.testAggregates.forEach((aggregate) => {
    aggregate.verified = testScores.get(aggregate.candidateId)
    aggregate.total = 50
  })
  const { report, files } = buildReportArtifacts(input)
  assert.equal(report.benchmark, 'hle-text-math')
  assert.equal(report.validationTotal, 50)
  assert.equal(report.testTotal, 50)
  assert.match(files['curve.svg'], /Validation \(50\)/u)
  assert.match(files['curve.svg'], /Hidden test \(50\)/u)
  assert.doesNotMatch(files['curve.svg'], /Hidden test \(172\)/u)
})
...
test('model-authored directions are literal in Markdown while JSON preserves the raw text', () => {
  ...
  input.mutations[0].direction = direction
  ...
  assert.equal(json.points[1].mutationDirection, direction)
...
test('CSV neutralizes spreadsheet formulas and curve JSON remains lossless', async (context) => {
  ...
  for (const direction of directions) {
    await context.test(JSON.stringify(direction), () => {
      ...
      const directionIndex = rows[0].indexOf('mutation_direction')
      ...
      assert.equal(JSON.parse(files['curve.json']).points[1].mutationDirection, direction)
    })
  }
})
...
test('reporting requires one matching Controller log entry for every mutation', async (context) => {
  await context.test('missing mutation', () => {
    const input = fixture()
    input.mutations.pop()
    assert.throws(() => buildCampaignReport(input), /validation failed/u)
  })
  await context.test('duplicate mutation', () => {
    const input = fixture()
    input.mutations.push({ ...input.mutations[0] })
    assert.throws(() => buildCampaignReport(input), /validation failed/u)
  })
  await context.test('mutation logged before evaluation', () => {
    const input = fixture()
    input.mutations[0].decidedAt = '2026-01-01T01:00:00Z'
    assert.throws(() => buildCampaignReport(input), /validation failed/u)
  })
})
